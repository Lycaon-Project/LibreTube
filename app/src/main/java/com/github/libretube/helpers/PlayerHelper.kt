...
object PlayerHelper {
    ...
    fun getFullscreenOrientation(isVerticalVideo: Boolean): Int {
        ...
        return when (fullscreenOrientationPref) {
            "ratio" -> {
                // probably a_youtube shorts video
                if (isVerticalVideo) {
    ...
    /**
     * Get the load controls for the player (buffering)
     */
    @OptIn(UnstableApi::class)
    ...
    /**
     * Get all caption tracks available in the player
     * Optimized using streams for better performance
     */
    ...
    /**
     * Get the currently played caption format, if any
     * Optimized to short-circuit as soon as a selected track is found
     */
    ...
    fun getFullAudioRoleFlags(roleFlags: Int, acontValue: String): Int {
        val acontRoleFlags = when (acontValue.lowercase()) {
            ...
            // nor the `xtags` one, so the track should be not set as the main one
    ...
    fun getVideoStats(tracks: Tracks, videoId: String): VideoStats {
        ...
        for (group in tracks.groups) {
            ...
            when (group.type) {
                C.TRACK_TYPE_AUDIO -> {
                    val audioFormat = (0 until group.length).firstOrNull { index ->
                ...
                C.TRACK_TYPE_VIDEO -> {
                    val videoFormat = (0 until group.length).firstOrNull { index ->
                        group.isTrackSelected(index)
                    }?.let { index -> group.getTrackFormat(index) } ?: continue
